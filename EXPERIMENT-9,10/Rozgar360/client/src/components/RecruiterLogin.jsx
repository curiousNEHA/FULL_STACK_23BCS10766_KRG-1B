...
const RecruiterLogin = () => {
 ...
 <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
       ...
       {state==='Login' ? 'Login' : isTextDataSubmited ? 'Create Account':'next'}
        ...
        {
            state === 'Login' ? 
             <p className='mt-5 text-center'>Don't have any account? <span className = 'text-blue-600 cursor-pointer' onClick = {()=>setState('Sign Up')}>Sign Up</span></p>:
   <p className='mt-5 text-center'>Already have an account? <span className = 'text-blue-600 cursor-pointer' onClick = {()=>setState('Login')}>Login</span></p>
        }
       <img onClick = {e=>setShowRecruiterLogin(false)} src = {assets.cross_icon} alt = "close" className='absolute top-5 right-5 cursor-pointer' />
