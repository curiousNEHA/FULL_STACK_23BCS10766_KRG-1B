    ...
    <>
      ...
      <div className = 'min-h-screen flex flex-col py-10 container px-4 2xl:px-20 mx-auto'>
        <div className='bg-white text-black rounded-lg w-full'>
          <div className='flex justify-center md:justify-between flex-wrap gap-8 px-14 py-20 mb-6 bg-sky-50 border border-sky-400 rounded-xl'>
            <div className = 'flex flex-col md:flex-row items-center'>
                <img className = 'h-24 bg-white rounded-lg p-4 mr-4 max-md:mb-4 border' src = {jobsData.companyId.image} alt = "company" />
        ...
        <div className='w-full lg:w-2/3'>
          ...
          <div className='w-full lg:w-1/3 mt-8 lg:mt-0 lg:ml-0 lg:ml-8 space-y-5'>
            <h2>More jobs from {jobsData.companyId.name}</h2>
            {jobs.filter(job=>job._id !== jobsData._id && job.companyId._id === jobsData.companyId._id).filter(job => true).slice(0,4).map((job,index)=><JobCard key = {index} job = {job}/>)}
          </div>
        </div>
        </div>
        </div>
